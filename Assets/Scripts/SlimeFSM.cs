...
public class SlimeFSM : MonoBehaviour
{
  ...
  enum MonsterState
    ...
    MonsterState m_State; // 몬스터 상태 변수

    Rigidbody2D rigid;   // Rigidbody2D 변수 : 오브젝트가 물리엔진 제어를 받도록

    public float findDistance =4.5f;  // 플레이어 발견 범위
    public float attackDistance = 1.7f; // 공격 가능 범위

    public Transform player;  // 플레이어 트랜스폼

    float currentTime = 0;  // 누적 시간
    float attackDelay = 6f;  // 공격 딜레이 시간

    public int attackPower = 7;   // 슬라임  공격력

    public int hp = 40;   // 몬스터 체력
    ...
    void Start()
    {
        ...
        Invoke("Think", 3f);
        m_State = MonsterState.Idle;  // 최초의 몬스터 상태 : 대기 
   ...
   void Think()
    ...
    void Attack()
    {
        ...
        if (Vector3.Distance(transform.position, player.position) < attackDistance)
        {
            ...
            {
                GameObject.Find("GameManager").SendMessage("LoseHP",attackPower);
                print("공격");
                currentTime = 0;
